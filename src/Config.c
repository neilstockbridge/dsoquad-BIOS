...
{
  GPIOA_CRL = 0x118088BB;  /* GPIOA Bit0-7 状态设置
                |||||||+----- Nib0  V1    复用输出 T51 PWM
                ||||||+------ Nib1  V2    复用输出 T52 PWM
                |||||+------- Nib2  NC    上拉输入  
                ||||+-------- Nib3  NC    上拉输入  
                |||+--------- Nib4  DAC1  模拟输出  
                ||+---------- Nib5  NC    上拉输入  
                |+----------- Nib6  SCK   低速输出      
                +------------ Nib7  SDA   低速输出*/    
  GPIOA_CRH = 0x88833BBB;  /* GPIOA Bit8-15 状态设置
                |||||||+----- Nib8  MCO   复用输出
                ||||||+------ Nib9  TX    复用输出
                |||||+------- Nib10 RX    复用输入  
                ||||+-------- Nib11 D-    复用输入  // 开机时暂时设为输出"0"
                |||+--------- Nib12 D+    复用输入  // 开机时暂时设为输出"0"  
                ||+---------- Nib13 K2    上拉输入  
                |+----------- Nib14 K1    上拉输入  
                +------------ Nib15 K8    上拉输入*/
  GPIOA_ODR = 0xE7FF;
...
//  GPIOB_CRL = 0x3BBBB433;  /* GPIOB Bit0-7 状态设置
  GPIOB_CRL = 0x3BBBB818;  /* GPIOB Bit0-7 状态设置
                |||||||+----- Nib0  NC    上拉输入  
                ||||||+------ Nib1  ClrW  低速输出
                |||||+------- Nib2  NC    上拉输入  
                ||||+-------- Nib3  MSCK  复用输出  
                |||+--------- Nib4  SIMO  复用输出  
                ||+---------- Nib5  MOSI  复用输出  
                |+----------- Nib6  Sqr   高阻输入/复用输出(T42 PWM) 
                +------------ Nib7  EECS  高速输出 */
  GPIOB_CRH = 0x88883338;  /* GPIOB Bit8-15 状态设置    0x83833333
                |||||||+----- Nib8  CHRG  上拉输入
                ||||||+------ Nib9  Bx0.1 高速输出
                |||||+------- Nib10 C_nSS 高速输出  // 配置成功后改为“标准版”PWRon 输出
                ||||+-------- Nib11 C_SCK 高速输出  
                |||+--------- Nib12 C_SI  高速输出  // 配置成功后改为“标准版”Ready 输入
                ||+---------- Nib13 C_SO  上拉输入  // 配置成功后改为“体验版”DTr   输入
                |+----------- Nib14 C_RST 高速输出  // 配置成功后改为“体验版”CTr   输入
                +------------ Nib15 Cdone 上拉输入*/ 
  GPIOB_ODR = 0xFFFF;
  
  GPIOC_CRL = 0xBB111000;  /* GPIOC Bit0-7 状态设置
                |||||||+----- Nib0  Ai    模拟输入
                ||||||+------ Nib1  Bi    模拟输入
                |||||+------- Nib2  Vbat  模拟输入  
                ||||+-------- Nib3  Mode  低速输出  
                |||+--------- Nib4  ClrR  低速输出 
                ||+---------- Nib5  H_L   低速输出  
                |+----------- Nib6  BL    低速输出 T81 PWM 
                +------------ Nib7  BZ    低速输出 T82 PWM*/
  GPIOC_CRH = 0x33388888;  /* GPIOC Bit8-15 状态设置
                |||||||+----- Nib8  Key_n 上拉输入
                ||||||+------ Nib9  Key_n 上拉输入
                |||||+------- Nib10 Key_n 上拉输入  
                ||||+-------- Nib11 Key_n 上拉输入  
  ...
  GPIOD_CRL = 0xB8BB84BB;  /* GPIOD Bit0-7 状态设置
                |||||||+----- Nib0  DB2   复用输出
                ||||||+------ Nib1  DB3   复用输出
                |||||+------- Nib2  Vusb  高阻输入  
                ||||+-------- Nib3  Key_n 上拉输入  
                ...
                |+----------- Nib6  Key_n 上拉输入  
                +------------ Nib7  nE1   复用输出*/
  GPIOD_CRH = 0xBB131BBB;  /* GPIOD Bit8-15 状态设置
                |||||||+----- Nib8  DB13  复用输出
                ||||||+------ Nib9  DB14  复用输出
                |||||+------- Nib10 DB15  复用输出  
                ||||+-------- Nib11 nCLR  低速输出  
  ...
  GPIOE_CRL = 0xB3333383;  /* GPIOE Bit0-7 状态设置
                |||||||+----- Nib0  AC2   高速输出
                ||||||+------ Nib1  NC    上拉输入
                |||||+------- Nib2  Bx1   高速输出  
                ||||+-------- Nib3  Ax0.1 高速输出  
                ...
                |+----------- Nib6  Ax2   高速输出  
                +------------ Nib7  DB4   复用输出*/
  GPIOE_CRH = 0xBBBBBBBB;  /* GPIOE Bit8-15 状态设置
                |||||||+----- Nib8  DB5   复用输出
                ||||||+------ Nib9  DB6   复用输出
                |||||+------- Nib10 DB7   复用输出  
                ||||+-------- Nib11 DB8   复用输出  
...
//  DMA1_CMAR1 =(u32)&Analog_Buff[0];//DMA通道1的源或目标的存储器地址
  ...
  DMA1_CCR1 |= 0x00000001;         // EN = 1：DMA1通道1开启 
...
//    TIM1 update also used for Sampling digital input and DMA1_CH5 
  ...
  TIM1_DIER =0x0000;/*0100 0001 0000 0000  DMA和中断使能寄存器
                       |     ||        +----CC1IE=0：不允许捕获/比较1中断
                       |     |+-------------UDE=1：允许更新的DMA请求
